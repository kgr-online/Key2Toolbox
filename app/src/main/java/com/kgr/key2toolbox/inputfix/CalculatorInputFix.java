...
public final class CalculatorInputFix implements InputFix {
    ...
    public boolean onKeyEvent(AccessibilityService service, KeyEvent event) {
        Key2KeyTranslator.Input input = Key2KeyTranslator.toCalculatorInput(event.getKeyCode());
        if (input == null) {
            return false;
        }
        ...
        try {
            ...
            String directText = Key2KeyTranslator.calculatorDirectText(input);
            if (directText != null) {
    ...
    private static AccessibilityNodeInfo findCalculatorButton(
            ...
            String packageName,
            Key2KeyTranslator.Input input
    ) {
        AccessibilityNodeInfo button = findFirstClickableNodeById(
                root,
                Key2KeyTranslator.calculatorButtonId(packageName, input)
        );
        if (button != null) {
            return button;
        }
        ...
        return AccessibilityNodes.findSingleNodeInTree(
                root,
                null,
                Key2KeyTranslator.calculatorButtonFallbackLabels(input)
        );
    }
