...
fun ExtraDimScreen(onBack: () -> Unit) {
    ...
    ScreenScaffold(title = stringResource(Screen.ExtraDim.titleRes), onBack = onBack) {
                ...
                onCheckedChange = { enable ->
                    ...
                    scope.launch(Dispatchers.IO) {
                        ...
                        statusMessage = context.getString(
                            if (enable) R.string.status_extra_dim_activated else R.string.status_extra_dim_deactivated
                        )
                    }
                }
            )
        ...
        Column(
            ...
            Text(stringResource(R.string.extra_dim_intensity, level.toInt()), style = MaterialTheme.typography.titleSmall)
            Slider(
                value = level,
                enabled = !busy,
                valueRange = 0f..100f,
                ...
                onValueChangeFinished = {
                    ...
                    scope.launch(Dispatchers.IO) {
                        ...
                        level = ExtraDimController.getDimmingLevel().toFloat()
                        busy = false
                        statusMessage = context.getString(R.string.status_extra_dim_intensity, level.toInt())
                    }
                }
            )
        }
        ...
        Text(
            if (scheduleEnabled && !scheduleRunning) stringResource(R.string.extra_dim_schedule_pending_boot, stringResource(R.string.generic_state_on))
            else stringResource(R.string.extra_dim_schedule_state, if (scheduleEnabled) stringResource(R.string.generic_state_on) else stringResource(R.string.generic_state_off))
        )
...
private fun TimePickerDialog(
    ...
    AlertDialog(
        ...
        confirmButton = {
            TextButton(onClick = { onConfirm(state.hour * 60 + state.minute) }) { Text(stringResource(R.string.generic_ok)) }
        },
        dismissButton = {
